...
//custom-product-slidermobile 
$('.pdp-m-slider-for').slick({
  slidesToShow: 1,
  slidesToScroll: 1,
  arrows: false,
  fade: true,
  asNavFor: '.pdp-m-slider-nav',
  infinite: false // Disable infinite loop to avoid cloning issues
});

$('.pdp-m-slider-nav').slick({
  slidesToShow: 5,
  slidesToScroll: 1,
  asNavFor: '.pdp-m-slider-for',
  dots: false,
  centerMode: false,
  focusOnSelect: true,
  infinite: false,       // prevent slide cloning
  variableWidth: false,  // disable variable width
});
//mobile scroll
function initSlick() {
  if ($(window).width() < 480) {
    if (!$('.more-abt-multicolumn2').hasClass('slick-initialized')) {
      $('.more-abt-multicolumn2').slick({
        speed: 300,
        dots: true,
        slidesToShow: 1,
        slidesToScroll: 1
    ...
    if ($('.more-abt-multicolumn2').hasClass('slick-initialized')) {
      $('.more-abt-multicolumn2').slick('unslick');
    }
  }
}
...
$.fn.initTabs = function(mobilegrid) {
  ...
  var firstPanel = _.find('[role="tabpanel"]').first().find('.ajaxcontent');
  getTabcontent(firstPanel, mobilegrid, function() {
    resizeTabImages(firstPanel);
  });
  firstPanel.data('loaded', true);

  // Bind tab clicks
  _.find('.nav-link').on('click', function(e) {
    e.preventDefault();
    var $this = $(this);
    var panel = $($this.attr('href')).find('.ajaxcontent');

    // Load only once per tab
    if (!panel.data('loaded')) {
      getTabcontent(panel, mobilegrid, function() {
        resizeTabImages(panel);
      });
...
// 🔄 Extracted image resize logic
function resizeTabImages(container) {
  container.find('[data-mainimage]').each(function() {
    var _ = $(this),
        wn = _.closest('.respimgsize').width(),
        w = wn + 'x',
        w2 = wn * 2 + 'x',
        img = _.attr('data-mainimage'),
        attr = _.attr('data-mainimageratio');

    if (img && img.includes('respimgsize')) {
      var i = img.replace('respimgsize', w);
      var i2 = img.replace('respimgsize', w2);
      _.attr('data-srcset', i + ' 1x,' + i2 + ' 2x')
       .attr('srcset', i + ' 1x,' + i2 + ' 2x');
    }

    if (!$('#same_product_height').length && attr) {
      var h = wn / Number(attr);
      _.parent().css({ height: h, display: 'block' });
    }
  });
}

// 🛠 Updated getTabcontent to accept callback
function getTabcontent(a, mobilegrid, callback) {
  ...
  jQuery.ajax({
    ...
    success: function(data) {
      ...
      // ✅ Ensure resize runs only after AJAX is done
...
function clickTabcontent(){
   console.log('Tab clicked:', $(this).text()); // 👈 Log tab text on click
  var $this = $(this);
  setTimeout(function(){
        ...
        currentAjaxRequest = $.getJSON(searchURL + '&view=json', function(data) {
          ...
          if(data.results_count == 0) {
            ...
            $.each(data.results, function(index, item) {
              // Skip the item if its title is "Gift Wrapping Paper"
              if (item.title === "Gift Wrapping Paper") {
                return true; // This will skip the current iteration and move to the next product
              }

              if(index >= 6) return false;
              var itemUrl = item.url.replace('collections/xmas-collection-1/', '');
              var link = $('<a></a>').attr('href', itemUrl);
