...
var addToCartHandler = {
  ...
  addItem: function(line_item) {
   ...
   // _.find('>a:first-child').attr('href', line_item.url);
    _.find('>a:first-child').replaceWith(_.find('>a:first-child').html());
     // Add the quantity input HTML after the tt-quantity and tt-price
    var cartQtyHtml = `
        <div class="cart-dr-qty">
            <label for="updates_${line_item.key}" class="visually-hidden">${line_item.product_title}</label>
            <div class="cart-qty-btn">
                <div class="qty-button icon-minus">-</div>
                <input id="updates_${line_item.key}" class="cart-qty-input" type="text" name="updates[]" value="${line_item.quantity}" min="1" pattern="[0-9]*">
                <div class="qty-button icon-plus">+</div>
            </div>
        </div>
    `;
  
   // _.find('.tt-price').after(cartQtyHtml);
    if (!line_item.product_title.includes("Free Gift")) {
    // Only append cartQtyHtml if the product title doesn't match your condition
    _.find('.tt-price').after(cartQtyHtml);
    }
      // Add link to the title (Product title)
    _.find('.tt-title').wrapInner('<a href="' + line_item.url + '" />');

    var _img = _.find('.tt-item-img').empty();
    $('<a href="' + line_item.url + '"><img src='+line_item.image+' alt="Cart Image1"></a>').appendTo(_img);
    ...
    _ul_children.length > 3 && $('.header-cart-more-message-js').show();
     // Call the updateCartinput function to initialize quantity buttons for this new item
   // updateCartinput();
...
Shopify.onCartUpdate = function(cart) {
  ...
  $('body').trigger('refreshCurrency');
console.log("cart1 update",cart);
 // showATC();
// Call fetchCart directly without setInterval
//fetchCart();
};
